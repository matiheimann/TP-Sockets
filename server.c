...
int main()
{
	...
	while ((clientDescriptor =
		    accept(serverDescriptor, (struct sockaddr *)&clientAddress,
			   &clientAddressSize))) {
		// Create thread
		pthread_t thread;
		...
		*connectionDescriptor = clientDescriptor;
		if (pthread_create(&thread, NULL,
				   (void *(*)(void *))connectionSolver,
				   (void *)connectionDescriptor) == -1) {
...
void *connectionSolver(void *connectionDescriptor)
{
	...
	executeSQLStatement("PRAGMA foreign_keys = ON;",
			    database); // Enable foreign keys in sqlite3

	char *message;
	int exit = 0;
	while (exit == 0) {
		memset(buffer, '\0',
		       MESSAGE_SIZE); // Clear buffer before reading
		...
		{
			flightNumber = buffer + 2;
			printf("%s%s\n", "Deleting flight number received: ",
			       flightNumber);
			if (flightExists(database, atoi(flightNumber))) {
		...
		if (startsWith(buffer, "F")) {
			...
			printf("%s%s\n", "Checking flight number state: ",
			       flightNumber);

			if (flightExists(database, atoi(flightNumber))) {
				selectSeatsFromFlight(
				    database, atoi(flightNumber), result);
				strcat(result, "\n");
				printf("%s\n%s\n",
		...
		if (startsWith(buffer, "validate user id: ")) {
			...
			if (userExists(database, userID)) {
				...
				addUser(database,
					userID); // Create new user in database
		...
		if (startsWith(buffer, "validate seat reservation: ")) {
			...
			printf(
			    "Validating seat reservation, seat:%d, flight:%d\n",
			    seatNumber, flight);
		...
		if (startsWith(buffer, "validate seat cancelation: ")) {
			...
			flight = atoi(flightNumber);
			printf(
			    "Validating seat cancelation, seat:%d, flight:%d\n",
			    seatNumber, flight);
