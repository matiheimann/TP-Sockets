...
int main(int argc, char const *argv[])
{
	...
	strncpy(serverAddress.sun_path, SOCKET_PATH,
		sizeof(serverAddress.sun_path));
	if (connect(serverDescriptor, (struct sockaddr *)&serverAddress,
	...
	while (newUserFlag == 0) {
		...
		if (strcmp(command, "y") == 0) {
			printf("%s\n", "Enter your new user id (integer of 3 "
				       "or less digits): ");
			do {
				memset(command, '\0', MESSAGE_SIZE);
				scanf("%s", command);
			} while (!isNewUserIDValid(
			    command, serverDescriptor)); // If userID is valid,
							 // it is created and
							 // added to the
							 // database
	...
	printf("%s\n", "Available commands: \n");
	printf("%s\n", "* Add flight (enter 'AFn', where n is the flight "
		       "number. Flight numbers cant have more than 3 "
		       "digits)\n");
	printf("%s\n", "* Delete flight (enter 'DFn', where n is the flight "
		       "number. Flight numbers cant have more than 3 "
		       "digits)\n");
	printf("%s\n", "* Check flight state (enter 'Fn' where n is the flight "
		       "number)\n  In the list presented after executing this "
		       "command, A stands for available and R stands for "
		       "reserved\n");
	printf("%s\n", "* Reserve a seat in a given flight (enter 'Rn' where n "
		       "is the flight number to see seats\n");
	printf("%s\n", "* Cancel a reservation in a given flight (enter 'Cn' "
		       "where n is the flight number to see seats\n");
	...
	while (exit == 0) {
		...
		if (startsWith(command, "C")) {
			...
			char *selectedFlight =
			    (char *)calloc(MESSAGE_SIZE, sizeof(char));
			strcpy(selectedFlight, command + 1);
			if (isValidFlightNumber(selectedFlight)) {
				strcat(aux, "F");
				strcat(aux, selectedFlight);
				write(serverDescriptor, aux,
				      strlen(aux)); // Write to server Fn
						    // command to see seat
						    // disposition
				memset(messageFromServer, '\0', MESSAGE_SIZE);
				read(serverDescriptor, messageFromServer,
				     MESSAGE_SIZE);
				printf("%s\n", messageFromServer);
				if (strcmp(messageFromServer,
					   "Flight does not exist\n") != 0) {
					printf("Enter which seat you would "
					       "like to cancel: ");
					do {
						memset(command, '\0',
						       MESSAGE_SIZE);
						scanf("%s", command);
					} while (!isValidSeatCancelation(
					    command, selectedFlight,
					    serverDescriptor));
					write(serverDescriptor, aux,
					      strlen(aux));
					memset(messageFromServer, '\0',
					       MESSAGE_SIZE);
					read(serverDescriptor,
					     messageFromServer, MESSAGE_SIZE);
					printf("%s\n", messageFromServer);
				}
			}
			free(aux);
		...
		if (startsWith(command, "AF")) {
			...
			if (isValidFlightNumber(flightNumber)) {
				write(serverDescriptor, command,
				      strlen(command));
				memset(messageFromServer, '\0', MESSAGE_SIZE);
				read(serverDescriptor, messageFromServer,
				     MESSAGE_SIZE);
...
int isUserCreated(char *userID, int serverDescriptor)
{
	...
	write(serverDescriptor, messageToServer,
	      strlen(messageToServer)); // Send user id to server for validation
	read(serverDescriptor, messageFromServer,
	     MESSAGE_SIZE); // read result from server
...
int isUserIDFormatCorrect(char *userID)
{
	...
	if (hasMoreThanThreeDigits(id)) {
		printf("%s\n", "Error, userID must be of equal or less than 3 "
			       "digits, try again:");
		return 0;
	}
...
int isValidFlightNumber(char *flightNumber)
{
	...
	if (hasMoreThanThreeDigits(atoi(flightNumber))) {
		printf("%s\n", "Error, flight number must be of equal or less "
			       "than 3 digits, try again:");
		return 0;
	}
...
int isValidSeatReservation(char *seat, char *flightNumber, int serverDescriptor)
{
	...
	write(
	    serverDescriptor, messageToServer,
	    strlen(
		messageToServer)); // Send seat number to server for validation
	read(serverDescriptor, messageFromServer,
	     MESSAGE_SIZE); // read result from server
